...
@app.on_message(filters.command(["create","craft","make"], prefixes=["/", ".", "!"]))
async def draw(_: app, m: t.Message):
    global Database
    prompt = getText(m)
    if prompt is None:
        return await m.reply_text("<code>ᴘʟᴇᴀsᴇ ᴘʀᴏᴠɪᴅᴇ ᴀ ᴘʀᴏᴍᴘᴛ. ᴜsᴀɢᴇ: /make <prompt></code>")
    user = m.from_user
...
async def selectModel(_:app,query:t.CallbackQuery):
    ...
    if promptData is None:
        return await query.edit_message_text("sᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ ᴡʀᴏɴɢ ʙᴀʙʏ !!.")
